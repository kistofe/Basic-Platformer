...
bool j1Fonts::Awake(pugi::xml_node& conf)
{
	...
	{
		const char* path = conf.child("default").attribute("path").as_string();
		int size;
		//large size
		size = conf.child("large_font").attribute("size").as_int();
		large_size = Load(path, size);

		//medium size
		size = conf.child("medium_font").attribute("size").as_int();
		medium_size = Load(path, size);

		//small size
		size = conf.child("small_font").attribute("size").as_int();
